...
class OfflineMemory:
    ...
    def _detect_and_store_names(self, text: str) -> None:
        ...
        # Detect user name: various patterns
        user_name_patterns = [
            ...
            r"this is\s+([a-zA-Z]+)",
            r"name's\s+([a-zA-Z]+)",
            r"they call me\s+([a-zA-Z]+)",
            r"people call me\s+([a-zA-Z]+)",
            r"you can call me\s+([a-zA-Z]+)",
        ]
        # Common words to exclude
        non_names = {"the", "a", "an", "very", "really", "just", "so", "here", "there", 
                     "fine", "good", "okay", "happy", "sad", "not", "yes", "no", "sure",
                     "tired", "hungry", "busy", "free", "back", "home", "done", "ready"}
            ...
            if match:
                ...
                if name.lower() not in non_names and len(name) >= 2:
                    ...
                    if getattr(self.config.retrieval, "debug", False):
                        print(f"[profile] Detected user name: {name.upper()}")
                    break
        
        # Detect assistant name: various patterns
        assistant_name_patterns = [
            r"your name (?:is|will be|shall be)\s+([a-zA-Z]+)",
            r"call you\s+([a-zA-Z]+)",
            r"i(?:'ll| will) call you\s+([a-zA-Z]+)",
            r"name you\s+([a-zA-Z]+)",
            r"naming you\s+([a-zA-Z]+)",
            r"you are\s+([a-zA-Z]+)",
            r"you're\s+([a-zA-Z]+)",
        ]
        for pattern in assistant_name_patterns:
            ...
            if match:
                name = match.group(1).capitalize()
                if name.lower() not in {"the", "a", "an", "very", "really", "just", "so", "my", "your"}:
                    self._update_profile(assistant_name=name.upper())
                    if getattr(self.config.retrieval, "debug", False):
                        print(f"[profile] Detected assistant name: {name.upper()}")
                    break
    ...
    def _index_entities_and_graph(self, memory_id: str, text: str, metadata: Optional[Dict[str, Any]], messages: Optional[List[Dict[str, str]]] = None):
        ...
        # CRITICAL: Link first-person pronouns to user_id for searchability
        # If memory contains "I", "My", "Me" and we have a user_id, add user_id as an entity
        user_id = metadata.get("user_id") if metadata else None
        if user_id:
            # Check if memory uses first-person pronouns
            first_person_patterns = [r"\bI\b", r"\bMy\b", r"\bme\b", r"\bI'm\b", r"\bI've\b", r"\bI'd\b"]
            import re as _fp_re
            has_first_person = any(_fp_re.search(p, text) for p in first_person_patterns)
            if has_first_person:
                # Add user_id as an entity so queries for "X" find "I" memories
                self.entities.add_memory_entities(memory_id, text, [user_id.capitalize()])
...
"""
    ...
    def search(self, query: str, *, user_id: Optional[str] = None, agent_id: Optional[str] = None, run_id: Optional[str] = None, limit: int = 10) -> Dict[str, Any]:
        ...
        # A* TRAVERSE ONLY - goal-directed retrieval (no undirected k-hop)
        # Traverse starts from user_id for comprehensive coverage
        related_memory_ids = set()
        traverse_nodes: List[Tuple[str, float]] = []
        rels_dicts = []  # Will be populated from traverse paths
        ...
        # Collect memory IDs from traverse results
        ...
        # Also collect from query entities directly (for direct matches)
        for e in q_entities:
            mids = self.entities.memories_for_entity(e)
            related_memory_ids.update(mids)
        
        # Cap memory IDs to prevent explosion (max 100)
        if len(related_memory_ids) > 100:
            related_memory_ids = set(list(related_memory_ids)[:100])
    ...
    def traverse(self, query: str, *, user_id: Optional[str] = None, max_hops: int = 4, limit: int = 5) -> Dict[str, Any]:
        ...
        # 1) Seed entity detection - PRIORITIZE user_id (all memories relate to user)
        # Start from user_id for comprehensive traversal
        if user_id:
            seed = user_id
        else:
            # Fallback to query entity detection
