...
def graph_only_search(mem: OfflineMemory, query: str, user_id: str, limit: int = 10, min_relevance: float = 0.3) -> dict:
    ...
    # Step 2: A* TRAVERSE ONLY - goal-directed retrieval (no undirected k-hop)
    related_memory_ids = set()
    
    # A* traverse from user_id for comprehensive coverage
    t_res = mem.traverse(query, user_id=user_id, max_hops=8, limit=30)
    ...
    # Also collect from query entities directly
    for e in q_entities:
        mids = mem.entities.memories_for_entity(e)
        related_memory_ids.update(mids)
    ...
    results = candidates[:20]  # Max 20 high-quality memories
...
def unified_memory_search(mem: OfflineMemory, query: str, user_id: str, limit: int = 10, debug: bool = False):
    ...
    try:
        ...
        # 6. Sort and Filter - Balanced accuracy filter (lowered from 0.4 for better recall)
        raw_candidates = [c for c in raw_candidates if c["score"] >= 0.3]  # Balanced accuracy filter
        raw_candidates.sort(key=lambda x: x["score"], reverse=True)
        results["results"] = raw_candidates[:20]  # Max 20 high-quality memories
...
def _get_memory_context(mem: OfflineMemory, results: list[dict], query_text: str, *, min_score: float = 0.3, max_k: int = 20) -> str:
