...
def main():
    ...
    parser.add_argument("--voice", action="store_true", help="Start in voice input mode (use @voice to toggle)")
    ...
    asr_input = None  # Lazy-loaded when needed
    
    def get_user_input():
        """Get input from user (text or voice based on mode)."""
        nonlocal voice_mode, asr_input
        if voice_mode:
            if asr_input is None:
                try:
                    from ASR.asr_wrapper import ASRInput
                    asr_input = ASRInput()
                    asr_input.start()
                except Exception as e:
            ...
            print("🎤 Listening...")
            return asr_input.get_next_utterance()
        else:
            return input("You: ").strip()
    
    if voice_mode:
        print("🎤 Voice mode enabled. Say '@voice' or type it to toggle back to text.")
    ...
    while True:
        try:
            ...
            # ================================================================
            # ACTION: @voice - Toggle voice input mode
            # ================================================================
            if action == "voice":
                voice_mode = not voice_mode
                if voice_mode:
                    print("🎤 Voice mode ON. Speak to input. Say '@voice' to toggle off.")
                    if asr_input is None:
                        try:
                            from ASR.asr_wrapper import ASRInput
                            asr_input = ASRInput()
                    ...
                    print("📝 Text mode ON. Type your message.")
                    if asr_input:
                        asr_input.stop()
                continue
            ...
            # ================================================================
            # Detect and store user/assistant names from conversation
            # This captures "my name is X", "call you Y", "I live in Z", etc.
            try:
                mem._detect_and_store_names(content)
            except Exception:
                pass
            ...
            # Rebuild system prompt with updated style/emotion and user/assistant names
            history[0]["content"] = compose_system_prompt(
                profile,
                user_name=mem._get_user_name(),
                assistant_name=mem._get_assistant_name()
            )
            ...
            # Add ORIGINAL content to history (not augmented) to prevent memory carryover
            history.append({"role": "user", "content": content})
            
            # Build temporary messages list for LLM with augmented CURRENT message only
            # This ensures memory context is fresh per-query and doesn't bleed into history
            messages_for_llm = history[:-1] + [{"role": "user", "content": augmented_content}]
            ...
            for token in mem.llm.generate_messages_stream(messages_for_llm, **gen_kwargs):
                print(token, end="", flush=True)
